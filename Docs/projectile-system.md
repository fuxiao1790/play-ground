...
- **Child-specific authoring:** `ProjectileChildSpawnConfig` carries full child projectile definition (shape, damage, speed, lifetime, pierce, visual) plus `ProjectileTrackingConfig` and `ProjectileChildSpawnBehavior` as optional tail parameters. Child spawning is now split into two classes: `ProjectileAttack` handles only the direct shot; `ChildSpawningProjectileAttack` holds inspector references to a parent `ProjectileAttack` and a child `ProjectileAttack` (both assigned by drag-and-drop prefab), reads all child projectile settings from the child prefab's serialized fields, and injects a `ProjectileChildSpawnConfig` into the parent via `SetChildConfig`. No fixed hierarchy required — the child prefab reference is not a scene child, so it is never discovered by `GetComponentsInChildren` in the loadout.
