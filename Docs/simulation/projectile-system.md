...
- **Data layout & events:** Projectile entities carry `ProjectileTag` plus common `CombatKinematicsComponent`, `CombatCollisionComponent`, and `CombatHitComponent` (now includes `CritChance`, `CritMultiplier`, and `SourceNodeId`). Projectile-only identity, lifetime, pierce/repeat-hit state, tracking, render, child-spawner, and recycle/spawn buffers stay in projectile components. Hits are written into the scope `CombatHitElement` buffer (defined in `Common`, shared with the AOE pipeline) and replayed by `ProjectileRoot` through the shared scene-facing `Hit` event (`CombatHitBatchHandler`). The `Hit` event fires **once per unique target per frame**, carrying all hits for that target in one `IReadOnlyList<CombatHitData>` batch. `CombatHitData` carries `Kind`, `Damage`, `Position`, `DirectDamageEnabled`, `StackEffect`, and `SourceNodeId`. `ICombatTarget.ReceiveHits` receives the same batch; its default implementation loops over the list and calls `ReceiveHit` per entry. Internal spawn-on-hit effects such as impact AOE/projectile spawns live in `CombatHitEffectElement` and are routed through `HitEffect`, not through the batch hit event. Do not expose `CombatHitPayloadElement` or `CombatHitEffectElement` to external scene listeners. Trigger-link snapshot types (`ProjectileTrackingConfig`, `ProjectileImpactAoeSnapshot`, `ProjectileImpactProjectileSnapshot`) live in `PlayGround.System.Common` so core effect data can reference them without a circular dependency (`Assets/Scripts/System/Common/CombatHitElement.cs`, `Assets/Scripts/System/Common/CombatEcsComponents.cs`, `Assets/Scripts/System/Projectile/ProjectileEcsComponents.cs`, `ProjectileRuntimeEvents.cs`).
- **Hit flush pipeline:** Collision workers each write to a thread-local `NativeQueue<CombatPendingHit>` via `ParallelWriter` (no cross-thread contention). `CombatHitFlushJob` runs single-threaded: it dequeues all entries, groups them by `CombatHitBucketKey(scope, targetId)` into a local `NativeParallelMultiHashMap`, then writes each group's hits contiguously into the scope `CombatHitElement` buffer. `ReplayAndClear` in `CombatHitReplay` scans the buffer in target groups (contiguous runs with the same `TargetId`), dispatching one `ReplayBatch` call per unique target. This reduces hit dispatch from O(total hits) to O(unique targets).
