...
- **Data layout & events:** Projectile entities carry `ProjectileTag` plus common `CombatKinematicsComponent` and `CombatCollisionComponent`; projectile hit data lives in `ProjectileHitComponent`. Projectile-only identity, lifetime, pierce/repeat-hit state, tracking, render, child-spawner, and recycle/spawn buffers stay in projectile components. Collision emits separate damage and spawn streams. Damage/status data is written into scoped `CombatDamageElement` buffers and replayed by `ProjectileRoot` through the scene-facing `Hit` event (`CombatHitBatchHandler`). `CombatHitData` carries `Kind`, `Damage`, `Position`, `DirectDamageEnabled`, `StackEffect`, and `SourceNodeId`. Internal spawn-on-hit effects such as impact AOE/projectile spawns live in `CombatSpawnElement` and route through `HitSpawn`, not through the batch hit event. Do not expose `CombatSpawnElement` to external scene listeners. Trigger-link snapshot types (`ProjectileTrackingConfig`, `ProjectileImpactAoeSnapshot`, `ProjectileImpactProjectileSnapshot`) live in `PlayGround.System.Common` so core effect data can reference them without a circular dependency (`Assets/Scripts/System/Common/CombatHitElement.cs`, `Assets/Scripts/System/Common/CombatEcsComponents.cs`, `Assets/Scripts/System/Projectile/ProjectileEcsComponents.cs`, `ProjectileRuntimeEvents.cs`).
- **Hit flush pipeline:** Collision workers write to separate `NativeQueue<CombatPendingDamage>` and `NativeQueue<CombatPendingSpawn>` streams via `ParallelWriter`. `CombatHitFlushJob` drains those queues into scoped `CombatDamageElement` and `CombatSpawnElement` buffers without target grouping or side-buffer index indirection. `CombatHitReplay` rolls damage only for damage events; spawn events are routed separately.
...
- **Impact AOE / hit spawns:** AOE and complex hit reactions should be added as explicit spawn payload data, then interpreted by internal spawn routing after `ProjectileRoot` drains spawn events. The collision and child spawn systems stay free of managed callbacks.
