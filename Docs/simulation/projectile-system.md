...
- **Data layout & events:** Projectile entities carry `ProjectileTag` plus common `CombatKinematicsComponent`, `CombatCollisionComponent`, and `CombatHitComponent` (now includes `CritChance`, `CritMultiplier`, and `SourceNodeId`). Projectile-only identity, lifetime, pierce/repeat-hit state, tracking, render, child-spawner, and recycle/spawn buffers stay in projectile components. Hits are written into the scope `CombatHitElement` buffer (defined in `Common`, shared with the AOE pipeline) and replayed by `ProjectileRoot` through the shared scene-facing `Hit` event with `CombatHitContext`. Generic replay payload data such as stack effects lives in `CombatHitPayloadElement`; internal spawn-on-hit effects such as impact AOE/projectile spawns live in `CombatHitEffectElement` and are routed through `HitEffect`, not through scene hit context. Do not expose `CombatHitPayloadElement` or `CombatHitEffectElement` to external scene listeners. Trigger-link snapshot types (`ProjectileTrackingConfig`, `ProjectileImpactAoeSnapshot`, `ProjectileImpactProjectileSnapshot`) live in `PlayGround.System.Common` so core effect data can reference them without a circular dependency (`Assets/Scripts/System/Common/CombatHitElement.cs`, `Assets/Scripts/System/Common/CombatEcsComponents.cs`, `Assets/Scripts/System/Projectile/ProjectileEcsComponents.cs`, `ProjectileRuntimeEvents.cs`).
