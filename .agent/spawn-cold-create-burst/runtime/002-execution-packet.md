...
## Goal
Move lingering AOE cold-create ECB recording from the managed main-thread loop into `LingeringAoeSpawnJob`.
...
- Lingering AOE component values, timed-spawn seed, pulse VFX, lifetime, arming, and enable bits.
- Main-thread ECB playback after job completion.
- Reuse counter records only reused disabled slots.

## Behavior To Change
- Use `Allocator.TempJob` for the lingering create ECB.
- Record unreused lingering AOE commands inside the Burst job.
...
- Task 001 has completed without changing shared AOE helper behavior.
- `LingeringAoeSpawnJob` exists in `Assets/Scripts/System/Aoes/AoeSpawnApplySystem.cs`.
- `AoeSpawnApplyUtility.RecordLingeringReset` is static and seeds lifetime, pulse VFX, timed spawn, render, active/collision, and arming state.

## Step-By-Step Instructions
- Change lingering `createEcb` allocator to `Allocator.TempJob`.
- Add `EntityCommandBuffer Ecb` and `EntityArchetype Archetype` to `LingeringAoeSpawnJob`.
- Pass `Ecb = createEcb` and `Archetype = _lingeringArchetype` into the job.
...
## Acceptance Criteria
- `LingeringAoeSpawnJob` compiles under Burst.
- Lingering reuse plus cold count equals command count.
