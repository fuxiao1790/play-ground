...
- Add swept projectile as a second archetype/lane, identified by non-enableable `SweptProjectileTag`.
- Membership authored by `ProjectileDefinition.sweptCollision`; never inferred from runtime speed.
- Swept and tracking are exclusive: compiler rejects final combination; swept archetype has no `ProjectileTrackingComponent`.
- Swept collision uses full-step oriented rectangle; nearest-first hits; no distance clamp or sweep config.

## Global Invariants
- ECS consumers require `ProjectileTag` plus applicable domain/lane tag.
- Reuse pool slots only inside matching archetype; discrete queries exclude swept tag.
...
- Expansion routes one event queue into discrete/swept command lists.
- Apply owns pool allocation/materialization; collision owns collision hit/event production.

## Data Flow
- Authoring flag -> compiled runtime -> `ProjectileSpawnCommand.SweptCollision` -> expansion command lane -> matching apply archetype.
...
- Explicit arming queries list `ArmingTag`; swept movement/collision exclude armed entities.

## Determinism Requirements
- Swept multi-hit candidates retained/resolved nearest-first by closest approach along origin-to-position segment.
...
- `SweptProjectileTag`, `ProjectileSweepComponent`, `CombatSweepMath`, authored flag/compile validation, swept command list/apply/movement/collision systems.
