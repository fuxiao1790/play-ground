...
- User decision (not in original plan): extend the `SortingGroup`-on-shared-parent proxy pattern to the mob spawn parent (`MobSpawnerRoot`'s `spawnParent`) and the player object too, for authoring consistency across all four tiers. Requires the player-tier and mob-tier `SortingGroup`s to share the exact same Sorting Layer + Order in Layer or the player/mob Y-sort invariant silently breaks — see index.md's "Post-Implementation Decision" section. Needs visual re-verification of Y-interleaving after authoring.
