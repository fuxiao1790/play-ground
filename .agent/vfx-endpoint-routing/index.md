...
producers (UNCHANGED)
  ...
  -> drain resolves (TypeId, Trigger) -> endpointId via flat table
...
- **Native/GPU handle ownership** — `Docs/coding-standards.md:292-313`. Every `GraphicsBuffer`
  ...
  (`CombatAoeVfxDispatcher.cs:126-137`) and on realloc (old buffer released before rebind).
...
- **No per-vfx-kind entity component / archetype** — user directive. One entity emits many vfx
  ...
  `(TypeId, Trigger) -> ...` mapping lives in a flat side table, not on entities. This plan
  conforms: the table maps to `endpointId`.
...
- **`EndpointIdByKey` flat table** (`key -> endpointId`, `-1` = no visual) — the only allowed
  home for the mapping under the no-archetype rule.
...
- **Refactor** (re-key the pool by asset; `(typeId, trigger)` becomes routing only):
  - resulting data flow: one dictionary keyed by graph identity; one flat routing table.
...
- **No archetype growth / no structural changes**: no component added to any combat entity;
  routing lives in a flat table read on the main thread. PASS — and strictly better than the
...
- **Ownership**: growth releases the old buffer before rebinding; `Dispose` and the `Register`
  rollback cover every endpoint buffer. PASS.
- **Visual-only**: payload unchanged. PASS.
- **Accidental drops removed**: capacity chases demand; remaining ceilings are the graph asset's
  authored particle Capacity and GPU memory, both honest. PASS.

## Tasks

- `001-endpoint-identity.md` — re-key the dispatcher pool by `VisualEffectAsset`; add
  `EndpointIdByKey` routing table; `Register` reuses an existing endpoint when the asset already
...
- `004-burst-bucketing.md` — **measurement-gated.** Move the O(events) main-thread drain into a
  Burst pass (flat lookup + counting sort into contiguous per-endpoint ranges). Only if profiling
  shows the drain matters.

001 and 002 are independent and each independently shippable. 003 depends on 001. 004 depends on
001 and should not start without a profile capture justifying it.
