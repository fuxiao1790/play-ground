...
Give the continuous archetype its own pool and apply system, and stop the discrete lane from
consuming continuous slots.
...
| `ProjectileContinuousTag` | — | **yes** |
| `ProjectileContinuousStepComponent` | — | **yes** |
| `ProjectileTrackingComponent` | yes | **no** |
| everything else | identical | identical |

The continuous archetype is *smaller*: it drops `ProjectileTrackingComponent` (bool + 4 floats +
...
the continuous archetype automatically. **Do not** add a `WithNone<ProjectileContinuousTag>` to
tracking — it would be redundant and imply the exclusion is a filter rather than a
structural fact.

## The highest-risk edit

`ProjectileTag` is on **both** archetypes, so `ProjectileDiscreteSpawnApplySystem._deadSlotQuery`
...
against continuous chunks that have no such component. That does not fail cleanly — it is a
...
([:294-336](../../Assets/Scripts/System/Projectiles/ProjectileDiscreteSpawnApplySystem.cs#L294-L336))
...
**`tracking[i] = cfg.Tracking` must stay out of `WriteCommon`** — the continuous lane has no
...
writing any continuous code.

### New system: `ProjectileContinuousSpawnApplySystem`

`Assets/Scripts/System/Projectiles/ProjectileContinuousSpawnApplySystem.cs`. Copy of
`ProjectileDiscreteSpawnApplySystem` with five differences:

1. **Archetype** — the list at
   [:49-67](../../Assets/Scripts/System/Projectiles/ProjectileDiscreteSpawnApplySystem.cs#L49-L67)
   **minus** `typeof(ProjectileTrackingComponent)`, **plus** `typeof(ProjectileContinuousTag)`
   and `typeof(ProjectileContinuousStepComponent)`.
2. **Dead-slot query** — `.WithAll<ProjectileTag>().WithAll<ProjectileContinuousTag>().WithDisabled<Active>()`.
3. **Command source** — `ContinuousCommands` instead of `Commands`; same `PendingHandle`.
...
5. **Seed the origin** — after `WriteCommon`:
   `sweeps[i] = new ProjectileContinuousStepComponent { Origin = cfg.Position };`

   Redundant on the normal path once `ProjectileContinuousOriginSystem` (task 006) runs every
...
`ProjectileContinuousSpawnApplySystem.*` prefix so the lanes are separable in the profiler, and
...
|---|---|---|
| `ProjectileTrackingSystem` | `ref`/`in ProjectileTrackingComponent` in `Execute` | continuous archetype lacks the component; self-excluding |
...
- Discrete `_deadSlotQuery` excludes `ProjectileContinuousTag`.
- Continuous archetype omits `ProjectileTrackingComponent` entirely.
- Tracking write stays out of the shared `WriteCommon`.
- Materialization refactor lands separately, behavior-neutral, tests green.
- `ProjectileContinuousStepComponent.Origin` seeded from `cfg.Position` on every reuse.
- Both lanes report spawn counts into `CombatStatsSingleton`.
- No `WithNone<ProjectileContinuousTag>` added to the tracking systems.

## Risks

- **Forgetting `WithNone`** — highest-consequence, lowest-visibility failure in the plan.
- **Cross-lane reuse is impossible by construction.** A continuous slot can never serve a
