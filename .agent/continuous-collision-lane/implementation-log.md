...
| Task | Status | Notes |
|---|---|---|
| 001-continuous-types.md | Complete | Declarations added. Static validation passed; compile check remains baseline-blocked by unrelated `TargetProxyUpdateEvent` source error in `CombatTargetProxy.cs`. |
| 002-corridor-geometry.md | **Reopened 2026-08-02** | Box must be the travel corridor only: `boxHalfExtents.x = dist * 0.5` with no along-travel extension, `BuildSweptBox` → `TryBuildTravelCorridor` returning `false` on a degenerate step, one `SupportExtent` call instead of two. See index Revision Log. |
...
| 006-step-origin-capture.md | **Reopened 2026-08-02** | Superseded. Previously completed as `006-swept-movement-system.md` (swept mover + discrete `WithNone`). Task redesigned: delete `SweptProjectileMovementSystem`, revert `ProjectileMovementSystem`, add `ProjectileContinuousOriginSystem`. See index Revision Log. |
| 007-continuous-collision-system.md | **Reopened 2026-08-02** | Continuous check must run *on top of* the discrete check, not instead of it: two `CombatCollisionMath.Hit` calls per candidate (discrete at `Position`, then corridor), and the broadphase query region becomes the union of the corridor AABB and `collision.BoundsMin/Max`. See index Revision Log. |
...
| 010-lane-naming-simulation.md | Complete | Added 2026-08-02. Pure rename to `Discrete`/`Continuous`. Drive from IDE symbol rename, not find/replace — the lane word changes position and `Commands` → `DiscreteCommands` has no `Swept` to match. |
| 011-lane-naming-authoring.md | Complete | Added 2026-08-02. Independent of 010. Contains the one serialization hazard: `ProjectileDefinition.continuousCollision`. |
| 012-lane-naming-tests-and-docs.md | Complete | Added 2026-08-02. Depends on 010 and 011. Includes the plan-directory rename, done last. |

## Completed Tasks

- 001-continuous-types.md (compile validation baseline-blocked; user directed continuation).
- 002-corridor-geometry.md (compile validation baseline-blocked; user directed continuation).
...
- ~~006-swept-movement-system.md~~ — superseded 2026-08-02, see 006-step-origin-capture.md.
- 007-continuous-collision-system.md (compile validation baseline-blocked; user directed continuation).
...
2. **006** — delete `SweptProjectileMovementSystem.cs`, remove `WithNone<ProjectileContinuousTag>` from `ProjectileMovementJob`, add `ProjectileContinuousOriginSystem`. Do **not** touch `ProjectileDiscreteSpawnApplySystem._deadSlotQuery`'s `WithNone` — different exclusion, still required.
...
2. **`ProjectileDefinition.continuousCollision` is a serialized field.** No `.asset` contains it
