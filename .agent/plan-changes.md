...
- **Additive vs refactor comparison**: Do not default to additive OOP changes. First compare an additive abstraction-preserving solution against a refactoring/data-path-collapsing solution. Prefer the solution with fewer runtime data paths, fewer copies, clearer ownership, and better cache/job behavior, even if it requires modifying existing types. Only add a new type if it removes complexity rather than hiding it.
