        ...
        public static void BeginSplit(SplitterState state, GUIStyle style, bool vertical, params GUILayoutOption[] options)
        {
            ...
            switch (Event.current.GetTypeForControl(state.ID))
            {
                case EventType.MouseDown:
                    ...
                    {
                        float num = (!guiSplitterGroup.isVertical) ? guiSplitterGroup.rect.x : guiSplitterGroup.rect.y;
                        ...
                        {
                            if (((!guiSplitterGroup.isVertical) ? new Rect(state.xOffset + num + state.realSizes[i] - (float)(state.splitSize / 2), guiSplitterGroup.rect.y, (float)state.splitSize, guiSplitterGroup.rect.height) : new Rect(state.xOffset + guiSplitterGroup.rect.x, num + state.realSizes[i] - state.splitSize / 2, guiSplitterGroup.rect.width, (float)state.splitSize)).Contains(Event.current.mousePosition))
                            {
                ...
                case EventType.Repaint:
                    {
                        float num4 = (!guiSplitterGroup.isVertical) ? guiSplitterGroup.rect.x : guiSplitterGroup.rect.y;
                        for (int j = 0; j < state.relativeSizes.Length - 1; j++)
                        {
                            Rect position = (!guiSplitterGroup.isVertical) ? new Rect(state.xOffset + num4 + state.realSizes[j] - (float)(state.splitSize / 2), guiSplitterGroup.rect.y, (float)state.splitSize, guiSplitterGroup.rect.height) : new Rect(state.xOffset + guiSplitterGroup.rect.x, num4 + state.realSizes[j] - state.splitSize / 2, guiSplitterGroup.rect.width, (float)state.splitSize);
