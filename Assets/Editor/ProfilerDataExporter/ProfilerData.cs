...
namespace ProfilerDataExporter
{
    ...
    public class ProfilerData
    {
        ...
        public string ToCsv()
        {
            ...
            sb.AppendLine("Frame,FunctionPath,Function,Total%,Self%,Calls,GCAlloc,TimeMs,SelfMs");
            for (int i = 0; i < frames.Count; ++i)
            {
                var frame = frames[i];
                for (int j = 0; j < frame.functions.Count; ++j)
                {
                    var fn = frame.functions[j];
                    sb.Append(i); sb.Append(',');
                    sb.Append(CsvEscape(fn.functionPath)); sb.Append(',');
                    sb.Append(CsvEscape(fn.GetValue(ProfilerColumn.FunctionName))); sb.Append(',');
                    sb.Append(fn.GetValue(ProfilerColumn.TotalPercent)); sb.Append(',');
                    sb.Append(fn.GetValue(ProfilerColumn.SelfPercent)); sb.Append(',');
                    sb.Append(fn.GetValue(ProfilerColumn.Calls)); sb.Append(',');
                    sb.Append(fn.GetValue(ProfilerColumn.GCMemory)); sb.Append(',');
                    sb.Append(fn.GetValue(ProfilerColumn.TotalTime)); sb.Append(',');
                    sb.AppendLine(fn.GetValue(ProfilerColumn.SelfTime));
                }
            }
            return sb.ToString();
        }
