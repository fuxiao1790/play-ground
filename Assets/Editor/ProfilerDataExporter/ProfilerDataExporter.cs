...
namespace ProfilerDataExporter
{
    ...
    public class ProfilerDataExporter : EditorWindow
    {
        ...
        private void DrawCsvFromFile()
        {
            EditorGUILayout.Space();
            GUILayout.Label("Export from .data File", EditorStyles.boldLabel);
            if (GUILayout.Button("Load .data → CSV", GUILayout.ExpandWidth(false)))
            {
                var dataPath = EditorUtility.OpenFilePanel("Select profiler .data file", "", "data");
                if (!string.IsNullOrEmpty(dataPath))
                {
                    var csvPath = EditorUtility.SaveFilePanel("Save CSV", Path.GetDirectoryName(dataPath), Path.GetFileNameWithoutExtension(dataPath), "csv");
                    if (!string.IsNullOrEmpty(csvPath))
                    {
                        ProfilerDriver.LoadProfile(dataPath, false);
                        pendingCsvPath = csvPath;
                        EditorApplication.delayCall += FlushPendingCsvExport;
                    }
                }
            }
        }

        private void FlushPendingCsvExport()
        {
            EditorApplication.delayCall -= FlushPendingCsvExport;
            ...
            {
                UnityEngine.Debug.LogError("ProfilerDataExporter: no frames loaded from .data file.");
                return;
            }
            var data = ProfilerData.GetProfilerData(first, last);
            File.WriteAllText(pendingCsvPath, data.ToCsv());
            data.Clear();
            UnityEngine.Debug.Log($"CSV exported: {pendingCsvPath}  ({last - first + 1} frames)");
        }
    }
